...
/**
 * Live article semantic (kNN) search against a real {@code gotham-media-browser} index (P8-T01).
 * Uses the deterministic {@link StubImageBindClient} so no ImageBind service is required; a unique
 * {@code section} filter isolates the seeded document, proving the kNN DSL and filters are accepted
 * by the live Serverless cluster. Skipped unless {@code ES_ENDPOINT} / {@code ES_API_KEY} are set.
 */
@Tag("integration")
class ArticleSemanticSearchServiceIT {
    ...
    @Test
    void semanticSearchReturnsFilteredCandidate() {
        StubImageBindClient embedder = new StubImageBindClient();
        ArticleRepository repository = new ArticleRepository(client, embedder);
        ArticleSemanticSearchService service = new ArticleSemanticSearchService(client, repository, embedder);
        String marker = "SEM-" + System.nanoTime();
        String section = "SemSection-" + marker;
