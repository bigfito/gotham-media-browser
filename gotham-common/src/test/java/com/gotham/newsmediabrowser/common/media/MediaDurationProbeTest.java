...
class MediaDurationProbeTest {
    ...
    void imagesHaveNoDuration() {
        assertThat(MediaDurationProbe.probe(MediaType.IMAGE, new byte[] {1, 2, 3})).isEmpty();
    }

    @Test
    void readsOneSecondWav() {
        byte[] wav = silentWav(8000, 8000);
        assertThat(MediaDurationProbe.probe(MediaType.AUDIO, wav))
    ...
    void readsMp4MvhdDuration() {
        ...
        assertThat(MediaDurationProbe.probe(MediaType.VIDEO, mp4))
                .contains(Duration.ofMillis(4500));
    }

    @Test
    void unknownAudioIsEmpty() {
        assertThat(MediaDurationProbe.probe(MediaType.AUDIO, new byte[] {0, 1, 2, 3})).isEmpty();
    }
