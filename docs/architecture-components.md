...
| Users / auth | Out of scope (single-user demo) |
...
- **UI:** Thymeleaf templates — browse, article detail, ingest/admin upload, search results with tabs:
  - Full-text
  - Semantic
  - Hybrid
  - Vector *(multimedia only)*
- **API/MVC:** Controllers for pages + form posts; services for ES and GCS.
...
sequenceDiagram
  ...
  User->>UI: Enter query + choose tab
  alt Full-text
    App->>ES: multi_match / BM25
  else Semantic / Hybrid / Vector
    App->>IB: embed query (text or media)
    IB-->>App: query vector
    App->>ES: knn and/or RRF hybrid
  end
  ES-->>App: hits
  App-->>UI: results page
```
